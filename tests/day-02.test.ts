...
const getRanges = (input: string) => input.split(',').map(range => range.split('-').map(Number) as [number, number])

describe('Day 2', () => {
	const example = getRanges(exampleData)
	const input = getRanges(inputData)
